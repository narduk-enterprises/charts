...
const capabilityCards = [
  { t: 'Interaction models', d: 'Line zoom on categories; candle zoom on time + indices; magnetic crosshair and HUD.', i: 'i-lucide-mouse-pointer-2' },
  { t: 'Linked layouts', d: 'v-model:domain across candles; map x-window to studies with time/index helpers.', i: 'i-lucide-link' },
  { t: 'Realtime buffers', d: 'Throttled candle and scalar streams with rolling windows for live desks.', i: 'i-lucide-radio' },
  { t: 'Theming', d: 'High-contrast, print, and colorblind-safe palettes plus explicit dark overrides.', i: 'i-lucide-contrast' },
  { t: 'Export & fullscreen', d: 'PNG rasterization with CSS inlining; browser fullscreen helper for tiles.', i: 'i-lucide-download' },
  { t: 'Performance hooks', d: 'Draw caps and aggregation utilities sized to plot width.', i: 'i-lucide-cpu' },
] as const
</script>
...
<template>
  <div role="main">
    <section class="relative overflow-hidden border-b border-default/60">
      <div class="pointer-events-none absolute inset-0 bg-linear-to-b from-muted/40 to-transparent" />
        ...
        <div class="grid items-center gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:gap-16">
          <div class="space-y-6">
            ...
            <p class="max-w-xl text-base leading-relaxed text-muted sm:text-lg">
              Vue 3 SVG components with explicit zoom models, synchronized time domains, rolling buffers, and deterministic studies—without canvas lock-in.
            </p>
            ...
            <div class="flex flex-wrap gap-4 text-xs text-muted">
              <span class="inline-flex items-center gap-1.5">
                <UIcon name="i-lucide-shield-check" class="h-4 w-4 text-primary" /> TypeScript public API
              </span>
              <span class="inline-flex items-center gap-1.5">
                <UIcon name="i-lucide-gauge" class="h-4 w-4 text-primary" /> SVG + accessibility defaults
              </span>
              <span class="inline-flex items-center gap-1.5">
          ...
          <UCard class="border-default/80 bg-default/50 shadow-lg ring-1 ring-default/40">
            <div class="space-y-3 p-2 sm:p-3">
              <div class="flex items-center justify-between gap-3">
                <div>
                  <p class="text-xs font-semibold uppercase tracking-wide text-muted">Live preview</p>
              ...
              <ClientOnly>
                ...
                />
                <template #fallback>
                  <div class="flex h-[340px] items-center justify-center rounded-lg border border-dashed border-default/60 text-sm text-muted">
                    Loading chart…
                  </div>
                </template>
              </ClientOnly>
    ...
    <section class="border-b border-default/60 py-14 sm:py-16">
      <UContainer>
        ...
        <p class="mt-4 max-w-3xl text-base leading-relaxed text-muted">
          <strong class="text-highlighted">@narduk-enterprises/narduk-charts</strong> is a focused Vue 3 charting kit for dashboards and trading terminals: candlesticks with volume and brush, multi-axis lines, bars, pie, scatter, histograms, and a stack primitive for linked panes. Indicators are pure functions you own—compute anywhere, render with the same SVG pipeline.
        </p>
      </UContainer>
    </section>
      ...
      <UContainer>
        ...
        <div class="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <UCard
            ...
            :key="cap.t"
            class="border-default/70 bg-muted/10"
          >
    ...
    <section class="border-b border-default/60 py-14 sm:py-16">
      <UContainer>
        <div class="flex flex-col justify-between gap-6 sm:flex-row sm:items-end">
          ...
          <UButton to="/showcase" color="primary" variant="soft" trailing-icon="i-lucide-arrow-right">
            Showcase index
          </UButton>
        </div>
    ...
    <section class="border-b border-default/60 py-14 sm:py-16">
      <UContainer>
        ...
        <p class="mt-4 max-w-3xl text-base text-muted">
          Install <code class="rounded bg-muted px-1.5 py-0.5 font-mono text-xs text-primary">@narduk-enterprises/narduk-charts</code>, import
          <code class="rounded bg-muted px-1.5 py-0.5 font-mono text-xs text-primary">@narduk-enterprises/narduk-charts/style.css</code>, and mount components with typed props and events.
          Subpath bundles (<code class="font-mono text-xs text-primary">/candle</code>, <code class="font-mono text-xs text-primary">/studies</code>, …) keep production splits predictable.
        </p>
    ...
    <section class="border-b border-default/60 py-14 sm:py-16">
      <UContainer>
        ...
        <p class="mt-4 max-w-3xl text-base text-muted">
          Explicit scope: alerts, DOM ladders, Pine script, and social feeds stay <em>out</em> of the core library so teams can integrate their own execution stack. What ships is chart geometry, interaction contracts, and export surfaces auditors can reason about.
        </p>
    ...
    <section class="py-16 sm:py-20">
      <UContainer>
        <UCard class="border-primary/25 bg-linear-to-br from-primary/10 via-default/80 to-muted/30">
          <div class="flex flex-col gap-6 p-2 sm:flex-row sm:items-center sm:justify-between sm:p-4">
            <div>
              <h2 class="text-xl font-semibold text-highlighted sm:text-2xl">
                Ship charts that match your desk’s quality bar.
              </h2>
              <p class="mt-2 max-w-xl text-sm text-muted">
                Start from the quick start, mirror any showcase in your sandbox, and trace types straight to the source on GitHub.
              </p>
            </div>
            <div class="flex flex-col gap-2 sm:items-end">
              <UButton size="lg" color="primary" to="/docs/quickstart" @click="captureCta('footer_cta_start')">
