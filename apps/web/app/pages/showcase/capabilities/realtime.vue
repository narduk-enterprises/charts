...
const { values, push } = useStreamingSeries(
  48,
  Array.from({ length: 48 }, (_, i) => 100 + Math.sin(i / 4) * 3),
  {
...
const lineSeries = computed(() => [
  { name: 'Live mid', data: values.value.map((v) => Number(v.toFixed(2))) },
])
...
<template>
  <UContainer class="py-10 sm:py-12">
    ...
    <p class="mt-3 max-w-3xl text-base text-muted">
      <code class="text-primary">useCandleStream</code> replaces the forming bucket when
    ...
    <div class="mt-8 space-y-8">
      <LiveDemoPane
        title="Streaming candles"
        description="Synthetic 1m append timer for this demo."
      >
        <NardukCandleChart
      ...
      <LiveDemoPane
        ...
        description="Pushes tweak the trailing window of the mid-price strip."
      >
        <NardukLineChart
          class="w-full min-w-0"
          :series="lineSeries"
    ...
    <div class="mt-10 flex flex-wrap gap-2">
      ...
      <UButton to="/docs/api/composables-streaming" color="neutral" variant="outline"
        >Composable API</UButton
      >
    </div>
  </UContainer>
</template>
