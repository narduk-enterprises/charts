...
const labels = bars.map(b => new Date(b.t).toLocaleDateString(undefined, { month: 'short', day: 'numeric' }))
...
const series = ref([
  ...
  { name: 'EMA 20', data: ema20.map((v: number | null) => (v === null ? null : Number(v.toFixed(2)))) },
])
...
<template>
  <UContainer class="py-10 sm:py-12">
    ...
    <p class="mt-3 max-w-3xl text-base text-muted">
      Indicators ship as deterministic TypeScript — no hidden WebWorker or canvas layer. Compute on the server or client, then bind the arrays into
      <code class="text-primary">NardukLineChart</code> or custom overlays. Subpath import keeps bundles lean:
      <code class="text-primary">import { rsi } from '@narduk-enterprises/narduk-charts/studies'</code>.
    </p>

    <div class="mt-8 space-y-8">
      <LiveDemoPane title="Price + EMA(20)" description="Close series with a nullable-aware EMA overlay.">
        <NardukLineChart
      ...
      <LiveDemoPane title="MACD(12,26,9)" description="Line chart renders MACD + signal columns from the same study output.">
        <NardukLineChart
          chart-title="MACD"
          class="w-full min-w-0"
          :series="macdSeries"
    ...
    <UButton class="mt-6" to="/docs/api/studies" color="primary" variant="soft">Studies API</UButton>
  </UContainer>
</template>
